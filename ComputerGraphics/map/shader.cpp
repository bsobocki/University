...
void Shader::setShaderTransformationUniforms(const glm::vec3& translationVec, float scale)
{
    use();
    ...
    setVec3("spherePos", translationVec);
}

void Shader::setProjectionAndView(Camera& camera, const float scr_width, const float scr_height)
{
   use();

   glm::mat4 projectionMat = glm::perspective(glm::radians(camera.Zoom), scr_width / scr_height, 0.1f, 100000.0f);
   setMat4("projection", projectionMat);
